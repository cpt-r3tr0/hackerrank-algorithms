...
void InQueue(int node, int mask, int cost) {
  ...
  */
  pair<int, pair<int, int>> current =
      make_pair(shortest_path[node][mask], make_pair(node, mask));
...
void Dijkstra(int src) {
  ...
  while (!queue.empty()) {
    ...
    for (const auto &neighbour : graph[src]) {
      InQueue(neighbour.first, current_shops | shops[neighbour.first],
              cost + neighbour.second);
    }
  }
}
void ClearStates() {
  ...
  for (int i = 0; i < (1 << fish_types_no); i++) {
    for (int j = i; j < (1 << fish_types_no); j++) {
      if ((i | j) == (1 << fish_types_no) - 1) {
        best_time = min(best_time, max(shortest_path[n_cities][i],
                                       shortest_path[n_cities][j]));
      }
    }
  }
  cout << best_time << endl;
