...
int boardCutting(vector<int> cost_y, vector<int> cost_x) {
    int n=cost_x.size(),m=cost_y.size(),i ;
   sort(cost_x.begin(),cost_x.end(),comp) ; sort(cost_y.begin(),cost_y.end(),comp) ;
   int l=0,r=0 ;
   ll ans=0 ;
    while(l<n && r<m)
    {
       if(cost_x[l]>=cost_y[r])
       {
             ans+= (ll)(r+1) * (ll)cost_x[l];
             l++ ;
       }
       else if(cost_x[l]<cost_y[r])
       {
             ans+= (ll)(l+1) * (ll)cost_y[r] ;
             r++ ;
       }
    }
    while(l<n)
    {
                ans+= (ll)(m+1) * (ll)cost_x[l];
                l++ ;
    }
    while(r<m)
    {
                ans+= (ll)(n+1) * (ll)cost_y[r] ;
                r++ ;
    }
    ans%=1000000007 ;
    return ans;
